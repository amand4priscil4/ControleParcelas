...
function listarCartoes() {
    ...
    result[0].values.forEach(([id, nome, bandeira]) => {
        ...
        const bandeiraEmoji = bandeira === 'visa' ? '💳' : bandeira === 'mastercard' ? '💳' : bandeira === 'elo' ? '💳' : '💳';
        div.innerHTML = `
            <span class="pessoa-nome">${bandeiraEmoji} ${nome} ${bandeira ? `<span class="badge badge-primary">${bandeira.toUpperCase()}</span>` : ''}</span>
...
// RELATÓRIO
document.getElementById('relatorio-mes').addEventListener('change', (e) => {
